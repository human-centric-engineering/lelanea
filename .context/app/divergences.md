...
| **Upstream status**  | Filed with Daybreak — see the issue linked from the PR that introduced this row. Reported as a defect with this repo as the reference implementation; we did not volunteer the platform's fix (`B7`).                                                                                                                                                                                                                                                                                    |
| **Deletion trigger** | Daybreak or Sunrise ships theme resolution that treats an absent `localStorage` value as "follow the system" rather than writing to it — either as a fix or behind a seam. Then both edits revert to upstream.                                                                                                                                                                                                                                                                           |
