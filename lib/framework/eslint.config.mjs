...
export default [
  ...
  // other, or a leaf route clears this ban and then trips the vocabulary scan.
  {
    ...
    ignores: [
      ...
      // The boot BRIDGE, named exactly rather than by directory: the exemption is
      // justified for this one composer, so anything else landing in `_framework/`
      // should NOT inherit it. Enforced, not merely asserted in prose — a second
      // file there fails lint on its first `@/lib/framework` import.
      'prisma/seeds/_framework/000-framework-boot.ts',
