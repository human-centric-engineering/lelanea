 ...
 *
 * The same holds for the drafted seed data in `seed-data/drafted/`
 * (voice, golden set, crisis resources, slot taxonomy, resources), which lived
 * in `content/` until §22 separated her words from what was drafted for her.
 * Moving them must not move them out of the boundary. §22's t-89 narrows this
 * to seeds only, once nothing at runtime reads either folder.
 *
  ...
  rules: {
    'no-restricted-syntax': [
      ...
      // first version of this rule, which matched only the two import shapes.
      {
        ...
        message:
          'Authored content is read through `@/lib/app/content`, not by re-exporting ' +
          'content/*.json — a re-export hands unvalidated JSON to every consumer of ' +
          'this module. See lib/app/content/index.ts.',
      },
      {
